...
std_msgs::UInt16 *walls;

void objectsCallback(visualization_msgs::MarkerArray){ //get object positions (sorted after value and distance)

}

void wallCallback(std_msgs::UInt16 msg){ //should contain width, height and all wall points = k*4 + 2 elements
    float size = sizeof(msg)*sizeof(*msg);
    walls = (std_msgs::UInt16*)malloc(size-2);
    memcpy(walls, msg, sizeoff(*walls)*(size-2);
    width = msg[0];
    height = msg[1];
}
...
void poseCallback(nav_msgs::Odometry msg){ // for re-calculation of the path when needed
    pose = msg;
    g_start.coord = {pose.pose.pose.position.y + offset[0] ,pose.pose.pose.position.x + offset[1]};
}
...
bool noCollision(std::vector<float> n2, std::vector<float> n1, float walls[nwalls][4]){         //array definition might be wrong
    ...
    for(int i =0 ; i<nWalls; ++i){
        ...
        dist = sqrt((wall[2]-wall[0])^2+(wall[3]-wall[1]^2);
        ...
        std::vector<float> P1 = {x1-n1*r , y1+n2*r};
        std::vector<float> P2 = {x1+n1*r , y1-n2*r};
        std::vector<float> P3 = {x2-n1*r , y2+n2*r};
        std::vector<float> P4 = {x2+n1*r , y2-n2*r};

        std::vector<float> C1 = P1;
        std::vector<float> C2 = P1;
        std::vector<float> C3 = P3;
        std::vector<float> C4 = P3;
        std::vector<float> D1 = P4;
        std::vector<float> D2 = P2;
        std::vector<float> D3 = P2;
        std::vector<float> D4 = P4;
...
void runRRT(){
    ...
    for(int i=0; i<numNodes; ++i){
        floor q_rand[] = {floor(randZO()*width), floor(randZO()*height)};
